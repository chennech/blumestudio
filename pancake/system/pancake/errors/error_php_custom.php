...
<div id="exception_error">
	<h1><span class="type"><?php echo $type ?> [ <?php echo $code ?> ]:</span> <span class="message"><?php echo "$message<br />$file:$line"; ?></span></h1>
	<div id="<?php echo $error_id ?>" class="content">
		
		<?php echo Pancake_Exceptions::debug_source($file, $line) ?>
		
		<ol class="trace">
			<?php foreach (Pancake_Exceptions::trace($trace) as $i => $step): ?>
			<li>
				<p>
					<span class="file">
					<?php if ($step['file']): $source_id = $error_id.'source'.$i; ?>
						<a href="#<?php echo $source_id ?>" onclick="return koggle('<?php echo $source_id ?>')"><?php echo Pancake_Exceptions::debug_path($step['file']) ?> [ <?php echo $step['line'] ?> ]</a>
					<?php else: ?>
				...
				<div id="<?php echo $args_id ?>" class="collapsed">
					...
					<?php foreach ($step['args'] as $name => $arg): ?>
						<tr>
							...
							<td><pre><?php echo TVarDumper::dump($arg); ?></pre></td>
						</tr>
					<?php endforeach ?>
					</table>
				</div>
	...
	<?php foreach (array('_GET', '_POST', '_CLEAN_SERVER') as $var): ?>
		...
		<div id="<?php echo $env_id ?>">
			<table cellspacing="0">
			<?php foreach ($GLOBALS[$var] as $key => $value): ?>
				<tr>
					<td><code><?php echo $key ?></code></td>
					<td><pre><?php echo TVarDumper::dump($value) ?></pre></td>
				</tr>
			<?php endforeach ?>
			</table>
		</div>
