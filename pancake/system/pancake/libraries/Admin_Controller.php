...
class Admin_Controller extends Pancake_Controller
{
	...
	public function __construct()
	{
		...
		{
			if ( ! $this->ion_auth->logged_in() and $this->method != 'no_internet_access' and $this->method != 'backend_css' and $this->method != 'backend_js' and $this->method != 'setup_js')
			{
...
(select count(*) from ".$this->db->dbprefix("tickets")." where assigned_user_id != {$this->current_user->id} or assigned_user_id IS NULL))";
		    $count_users_sql = "(select count(*) from ".$this->db->dbprefix("users").")";
                    ...
                    $this->template->update_counter = count($this->update->get_versions_between_inclusive()) - 1;
