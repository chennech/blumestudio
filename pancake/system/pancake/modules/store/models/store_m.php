...
class Store_m extends Pancake_Model {
    ...
    function get_plugins($per_page, $offset, $term = '') {
        $results = json_decode($this->http->request($this->base . 'get_plugins/' . $per_page . "/" . $offset, 'POST', array(
                    'term' => $term
                )), true);
    ...
    function get_single($unique_id) {
        $result = json_decode($this->http->request($this->base . 'get_single_plugin/' . $unique_id), true);
