...
class Pie_m extends Pancake_Model {
    ...
    function csv_to_array($filename, $has_headers = true) {
        ...
        include_once APPPATH . 'modules/settings/libraries/parsecsv.lib.php';
        ...
        $fields = array_keys(reset($csv->data));
        $records = $csv->data;

        $last = $records;
        $last = end($last);
        $reset_last = reset($last);
        if (count($last) == 1 and empty($reset_last)) {
            array_pop($records);
        }
    ...
    function prepare_import($type, $filename, $ext) {
        ...
        if ($this->$method($buffer['fields'], $buffer['records'])) {
            ...
            if (IS_DEBUGGING and $ext != "csv") {
                debug("The \$fields below didn't match any known format in $method.", $buffer['fields']);
            }
            return false;
        }
    }
    ...
    function process_clients_txt(&$fields, &$records) {
        ...
        if ($fields == $bamboo[0] or $fields == $bamboo[1]) {
            ...
            case 'estimates':
                ...
                foreach ($import['records'] as $record) {
                    ...
                    if ($invoice == 'EXISTS') {
                        ...
                        $record['invoice_number'] = $this->invoice_m->_generate_invoice_number(null, "ESTIMATE");
                    }
                ...
                foreach ($import['records'] as $record) {
                    if (isset($record['original_from_pancake']) and $record['original_from_pancake']) {
                        ...
                        if ($invoice == 'EXISTS') {
                            ...
                            $record['invoice_number'] = $this->invoice_m->_generate_invoice_number();
                        }
    ...
    function prepare_export($type, $ext) {
        ...
        $filename = FCPATH . 'uploads/exports/' . date('Y-m-d') . '/' . $type . '-' . date('H-i-s') . '-' . crc32(uniqid()) . '.' . $ext;
    ...
    function get_export_csv(&$fields, $records, $type) {
        $filename = $this->prepare_export($type, 'csv');
    ...
    function get_export_json($records, $type) {
        $filename = $this->prepare_export($type, 'json');
