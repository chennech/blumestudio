        ...
        if (isset($input['name'])) {
            ...
            foreach ($input['name'] as $plugin) {
                if (isset($input['cb'][$plugin])) {
                    $this->plugins_m->set_plugin_setting($plugin . '_installed', true);
                } else {
                    $this->plugins_m->set_plugin_setting($plugin . '_installed', false);
                }
            }
        }

        if (isset($input['field'])) {
            foreach ($input['field'] as $k => $val) {
                $this->plugins_m->set_plugin_setting($k, $val);
            }
        }
