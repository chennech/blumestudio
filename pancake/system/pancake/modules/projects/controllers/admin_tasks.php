...
class Admin_Tasks extends Admin_Controller {
	...
	public function edit($task_id = null)
	{
		...
		if ($_POST)
		{
			...
			$_POST['due_date'] = read_date_picker($_POST['due_date']);
	...
	public function delete($task_id)
	{
		...
		if (IS_AJAX) output_json($message);
	...
	public function discussion($task_id)
	{
		$this->load->model('kitchen/kitchen_comment_m');
		$this->load->model('clients/clients_m');

		$task = $this->project_task_m->get_task_by_id($task_id);
		if (!$task or $task->num_rows() == 0) {
                    redirect("admin/projects");
                }
		$task = $task->row();
		$project = $this->project_m->get($task->project_id);
		...
		foreach ($comments as $key => $comment)
		{
			if ($comment->user_id)
			{
				$comment->user = $this->user_m->get_by_id($comment->user_id);
			} elseif ($comment->client_id != '0') {
				$comment->client = $this->clients_m->getById($comment->client_id);
			}
		}

		$this->template->chatters = $this->project_task_m->get_chatters($task_id);

		$client = $this->clients_m->getById($project->client_id);

		$this->template->client = $client;
		$this->template->client_id = $project->client_id;
		$this->template->comments = $comments;
		$this->template->task = $task;
		$this->template->build('discussion', array('task_id' => $task_id));
	}

	public function discuss()
	{
		$task = $this->project_task_m->get_task_by_id($this->input->post('task_id'));
		...
		if (!$_POST || !$task || !$project) show_404();
	 	$this->load->model('kitchen/kitchen_comment_m');

	 	$data = array(
	 		'item_type' => 'task',
	 		'item_id' => $this->input->post('task_id'),
	 	...
	 	$comment_id = $this->kitchen_comment_m->insert_comment($data, @$_FILES['files']);

		if (isset($_FILES['files']))
		{
			$this->load->model('kitchen/kitchen_files_m');
			$this->kitchen_files_m->upload($_FILES['files'], $comment_id, $project->client_id);
		}

		redirect('admin/projects/tasks/discussion/'.$task->id);
	}
