...
class Project_task_m extends Pancake_Model
{
	...
	public function get_tasks_by_project($project_id = null, $limit = null, $offset =  null, $parent_id = null, $all = false)
	{
		...
		$this->db
			...
			if ($parent_id)
			{
				...
				$this->db->where($this->db->dbprefix($this->tasks_table).'.parent_id', 0);
			}

			$this->db->order_by('order asc, project_milestones.id, project_milestones.target_date, completed ASC, id ASC')
			->group_by('')
			->limit($limit, $offset);
    ...
    public function upcoming_tasks_for_user($user_id, $x = 6) {
        ...
        $sql = "
SELECT pt.completed, pt.name AS task_name, pt.is_viewable, pt.projected_hours, pt.notes, pt.id AS task_id, pt.due_date AS task_due_date,
        pt.status_id,
...
sql;
	...
	public function get_chatters($task_id)
	{
		$comments_table = $this->db->dbprefix('comments');
		$sql = <<<sql
select DISTINCT(user_id) from $comments_table where item_id = $task_id
and item_type = 'task'
sql;
		...
		return $this->user_m->get_users_by_ids($uids);
	}
        ...
        public function get_for_billing($existing_invoice_rows = array()) {
            ...
            if (count($time_entries) == 0) {
                ...
                foreach ($buffer as $row) {
                    ...
                    if (!isset($milestone_time_entries[$milestone_id])) {
                        $milestone_time_entries[$milestone_id] = array();
                    }

                    $milestone_time_entries[$milestone_id] += $time_entries[(int) $row['project_id']][(int) $row['id']];
                    $milestone_ids[$milestone_id] = $row['project_id'];

                    $return[(int) $row['project_id']]['tasks']->{$task_id} = $row;
                    $return[(int) $row['project_id']]['milestones']->{$milestone_id} = $row;
                }

                $CI = get_instance();
                $CI->load->model('projects/project_milestone_m');
                $milestones = $CI->project_milestone_m->get_milestones($milestone_ids);
                foreach ($milestones as $id => $row) {
                    # Cast to object for JSON purposes.
                    $row['time_entries'] = (object) $milestone_time_entries[$row['id']];
	...
	public function count_all_tasks($project_id = null, $milestone_id = null)
	{
        ...
        function update($primary_value, $data, $skip_validation = FALSE) {
            $data['assigned_user_id'] = isset($data['assigned_user_id']) ? (int) $data['assigned_user_id'] : 0;
	...
	public function set_assigned_user($task_id, $assigned_user_id) {
		...
		$this->update($task_id, array('assigned_user_id' => $assigned_user_id));
                ...
                $task = array_reset($this->get_tasks_by_project());
	...
	public function insert_task($input)
	{
		...
		$result = $this->db->insert($this->tasks_table, array(
                    'owner_id' => current_user(),
                    'project_id' => $input['project_id'],
                    'name' => $input['name'],
                    'due_date' => !empty($input['due_date']) ? read_date_picker($input['due_date']) : 0,
                    'notes' => !empty($input['notes']) ? ($input['notes']) : '',
                    'parent_id' => ($input['parent_task_id'] == '') ? 0 : $input['parent_task_id'],
                    'rate' => $input['rate'],
                    'is_flat_rate' => isset($input['is_flat_rate']) ? $input['is_flat_rate'] : 0,
                    'completed' => 0,
                    'milestone_id' => $input['milestone_id'],
                    'projected_hours' => isset($input['projected_hours']) ? (int) time_to_decimal($input['projected_hours']) : 0,
	...
	public function update_task($task, $ignore_is_viewable = false)
	{
		...
		//empty($task['due_date']) OR $task['due_date'] = read_date_picker($task['due_date']);
    ...
    function quick_add($name, $project_id, $milestone_id = null, $assigned_user_id = null) {
        ...
        if ($result && $assigned_user_id) {
            $result = $this->project_task_m->set_assigned_user($insert_id, $assigned_user_id);
        }
