...
class Ticket_m extends Pancake_Model
{
	...
	{
		$this->db->select('clients.first_name client_first_name, clients.last_name client_last_name, clients.email client_email');
    ...
    public function generate_invoice_and_send_notification($ticket_id, $status_id) {
        ...
        if ($CI->client_support_rates_matrix_m->is_billable($ticket['priority_id'], $ticket['client_id'])) {
            if ($status_id > 0 and $status_id == Settings::get("ticket_status_for_sending_invoice")) {
                ...
                $invoice = $this->invoice_m->get_by_unique_id($invoice_id['uid']);
                $this->invoice_m->update_simple($invoice_id['uid'], array('last_sent' => time(), 'has_sent_notification' => 1));
                $this->ticket_m->sendNotificationEmail('email_new_ticket_invoice', $ticket_id, FALSE, $invoice);
            }
        }
    }
    ...
    public function sendNotificationEmail($type, $ticket_id, $admin = FALSE, $invoice = FALSE) {
        ...
        $result = send_pancake_email(array(
            'to' => !$admin ? $client['email'] : $user->email,
