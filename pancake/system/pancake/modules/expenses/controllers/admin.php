...
class Admin extends Admin_Controller
{
	...
	public function create()
	{
		if ($_POST)
		{
			...
			{
				$insert['due_date'] = date('Y-m-d', $dueDate / 1000);
			}
	...
	public function edit($expense_id)
	{
		...
		if ($_POST)
		{
			$result = $this->expenses_m->update($expense_id, array(
				...
				'supplier_id' => $this->input->post('supplier_id'),
				'category_id' => $this->input->post('category_id'),
				'due_date' => date('Y-m-d', $this->input->post('due_date')/1000),
                                'receipt' => $receipt,
			));
	...
	public function sort()
	{
		...
		$start = read_date_picker($this->input->get('start_date'));
		$end = read_date_picker($this->input->get('end_date'));
