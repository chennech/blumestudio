function get_invoice_total(ignore_index) {
    ...
    $('#invoice-items .details').each(function () {
        ...
        var discount = Billing.parse_amount_or_percentage(el.find('.item_discount').val(), qty * rate);
        ...
        if (type === "fixed_discount" || type == "percentage_discount") {
            ...
            items.push({
                ...
                qty: qty,
                rate: rate,
                total_pre_tax_pre_discounts: (qty*rate) - discount,
                total_pre_tax_post_discounts: (qty*rate) - discount,
                total_post_tax_post_discounts: (qty*rate) - discount,
                discount: discount,
                taxes: taxes
            });
        }
...
$(function () {
    ...
    $('input.partial-amount, input.item_quantity, input.item_rate').livequery(function () {
        $(this).forceNumeric();
    });
...
var Billing = {
    init: function () {
        ...
        $('.js-invoice-form')
                .on("keyup", "input.item_quantity, input.item_rate, input.item_discount", Billing.update_total)
                ...
                .on('change keyup', 'input.partial-amount, select[id^=partial-percentage], #invoice-items .item_quantity, #invoice-items .item_rate, #invoice-items .item_discount', updatePaymentPlanTotals)
    ...
    update_total: function () {
        ...
        if (type == "fixed_discount") {
            ...
            cost = (get_invoice_total(index) * (discount / 100)).toFixed(2);
        } else {
...
(function ($, window, document) {
    ...
    $(document).ready(function() {
        ...
        $("select, input, textarea").on('focus', function () {
            ...
            if (value !== previous) {
                console.log("change", $(this).attr("name"), value, previous);
                is_submitting = false;
            }
        });
    });
