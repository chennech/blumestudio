...
<?php if (count($entries)): ?>
    ...
    <div id="sort-entries-fields" class="row">
        ...
        <a href='<?php echo site_url('timesheet/' . $project->unique_id); ?>' class='blue-btn'><?php echo __("timesheet:view_for_clients"); ?></a>
        <br/><br/>
        ...
        <div class="height_transition">
            <div class="view_entries_table">
                <table id="view-entries" class="listtable pc-table table-activity" style="width: 100%;">
                    ...
                    <tfoot>
                    <tr>
                        <td colspan='2' rowspan='<?php echo $total_tasks ?>' class='align-right'><?php echo __("tasks:total_logged_time"); ?></td>
                        <?php if ($total_tasks > 1): ?>
                            <td class='align-right'><?php echo isset($tasks_select[$first_task]) ? $tasks_select[$first_task] : __('tasks:no_task'); ?></td><?php endif; ?>
                    ...
                    <?php foreach ($entries as $entry): ?>
                        ...
                        <tr data-id="<?php echo $entry->id ?>">
                            ...
                            <td class="cell2 user">
                                ...
                                <span class="time-sheet-name"><?php echo $entry->first_name . " " . $entry->last_name; ?>
                            </td>

                            <?php if ($total_tasks > 1): ?>
                                <td><?php echo isset($tasks_select[$entry->task_id]) ? $tasks_select[$entry->task_id] : __('tasks:no_task'); ?></td><?php endif; ?>
                            ...
                            <td class="cell4 duration nowrap">
                                <span class="value"><?php echo format_seconds($entry->minutes * 60); ?></span>
                                <small>(
            ...
            <div style="display:none;" class="row edit-entry edit-entry-<?php echo $time->id; ?>">
                ...
                <div class="three columns mobile-two">
                		<div class="row">
                				...
                				<?php echo form_input('start_time', set_value('start_time', isset($time) ? $time->start_time : ''), 'id="start_time_' . $time->id . '" class="txt five columns"'); ?>
                ...
                <div class="three columns mobile-two">
                		<div class="row">
                				...
                				<?php echo form_input('end_time', set_value('end_time', isset($time) ? $time->end_time : date('H:i')), 'id="end_time_' . $time->id . '" class="txt five columns"'); ?>
                				<div class="three columns time"></div>
                		</div><!-- /row -->
                </div><!-- /3 -->
                ...
                <div class="twelve columns">
                	<input type="hidden" name="project_id" value="<?php echo $project_id; ?>" />
                		<a href="#" class="blue-btn js-fake-submit-button"><span><?php echo __('tasks:edit_entry'); ?></span></a>
                		<input type="submit" class="hidden-submit" />
                </div>
           	</div><!-- /none -->
			</form>
