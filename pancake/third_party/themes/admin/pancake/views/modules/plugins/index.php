        		...
        		<div id="_<?php echo $plugin->alias; ?>">
        			...
        			<p>
        				<a href="<?php echo $plugin->url; ?>"><?php echo $plugin->url; ?></a>
                                ...
                                <?php if (isset($plugin->description) and !empty($plugin->description)): ?><div style="margin-bottom: 1em;"><?php echo $plugin->description; ?></div><?php endif;?>
        			<div class="row">
          					...
          					<div class="twelve columns">
          						...
          						<?php
          							switch($field_data['type']){
          								...
          								<?php break;
                                                                        case 'textarea': ?>
          										<textarea name="field[<?php  echo $field_name  ?>]"><?php echo empty($field_data['value']) ? $field_data['default'] : $field_data['value']; ?></textarea>
          								<?php break;
                                                                        case 'select': ?>
                                                                                        <div class="sel-item">
                                                                                            <select name="field[<?php  echo $field_name  ?>]" >
                                                                                                <?php $options = is_callable($field_data['options']) ? call_user_func($field_data['options']) : $field_data['options']; ?>
                                                                                                <?php foreach ($options as $value => $label): ?>
                                                                                                    <option value="<?php echo $value; ?>" <?php echo ($field_data['value'] == $value) ? 'selected="selected"' : ''; ?>><?php echo $label;?></option>
                                                                                                <?php endforeach; ?>
                                                                                            </select>
                                                                                        </div>
          								<?php break;
