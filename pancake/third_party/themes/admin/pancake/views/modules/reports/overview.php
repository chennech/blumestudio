<div class="six columns">
  <h4 class="ttl ttl3"><?php echo $title; ?> <span class="report-per"><?php echo $per;?></span> <span class="report-daterange"><?php echo __("reports:verb_from_to", array($verb, format_date($from), format_date($to)));?></span></h4>
    ...
    <?php if (is_array($chart_totals) and count($chart_totals) > 0) :?>
        <?php if ($chart_type == 'pie') :?>
            ...
            foreach ($chart_totals as $label => $total) {
                if ($i > 5 and ((($total/$report_total)*100) < $percent_threshold)) {
